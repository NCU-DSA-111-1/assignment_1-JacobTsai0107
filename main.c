...
int n = 1;/*Counter for calculating cost function*/
int num_training_ex;
int main(void)
{
int i;/*Counter for main function to input the number of layers and neurons*/
srand(time(0));
fptr1 = fopen("iteration_times.txt","w");
fclose(fptr1);
fptr2 = fopen("total_cost.txt","w");
fclose(fptr2);
printf("Enter the number of Layers in Neural Network(including input layer and output layer):\n");
scanf("%d",&num_layers);
//Check if the number of layers is enough or not
while(1){
    if(num_layers<3){
        printf("please do not set the number less than 3:\n");
        scanf("%d",&num_layers);
    }
    else{
        break;
...
// Get number of neurons per layer and let the number of neurons at output layer be 1
...
// Check if the architecture is successful or not
int init()
{
...
void get_inputs()
{
int i,j;/*Counter to get input from training data*/
for(i=0;i<num_training_ex;i++)
{
...
{
int i,j;/*Counter to get output of training data*/
for(i=0;i<num_training_ex;i++)
{
...
{
int i=0,j=0;/*initialize counter*/
...
{
// Initialize Output Weights for each neuron in random
...
int i;
int it=0;
// Iteration times equal to epochs
...
fptr1 = fopen("iteration_times.txt","a");
fprintf(fptr1,"%d\t",it+1);
fclose(fptr1);
fptr2 = fopen("total_cost.txt","a");
fprintf(fptr2,"%f\t",full_cost);
fclose(fptr2);
}
}
...
// Compute Total Cost using mean absolute error
...
full_cost = (full_cost * (n-1) + tcost)/n;
 // printf("Full Cost in %d times iteration: %f\n\n", n, full_cost);
n++;
}
// Back Propogation with gradient descent to minimize error
...
desired_outputs[p][j]) * (lay[num_layers-1].neu[j].actv) * (1-lay[num_layers-1].neu[j].actv);
...
void test_nn(void)
{
int i;/*Counter for testing input*/
while(1)
{
